...
code=$(req GET /tenants/acme/roles "$ALICE")
...
check "admin lacks tenant.delete" "false" "$(jq '[.roles[]|select(.key=="admin")][0].permissions|any(.=="tenant.delete")' /tmp/body)"
...
code=$(req POST /tenants/acme/invitations "$ALICE" "{\"email\":\"mallory@example.com\",\"role_id\":\"$ADMIN_ID\"}")
check "invite mallory as admin" 201 "$code"
TOK=$(invite_token); req POST /invitations/accept "$MALLORY" "{\"token\":\"$TOK\"}" >/dev/null
code=$(req POST /tenants/acme/invitations "$ALICE" "{\"email\":\"bob@example.com\",\"role_id\":\"$MEMBER_ID\"}")
...
code=$(req GET /tenants/acme/audit "$BOB")
check "member reading audit -> 403 (lacks audit.read)" 403 "$code"
code=$(req GET /tenants/acme/roles "$BOB")
check "member listing roles -> 403 (lacks roles.read)" 403 "$code"
code=$(req GET /tenants/acme/audit "$MALLORY")
check "admin reading audit -> 200 (has audit.read)" 200 "$code"

echo "== ESCALATION: mallory is an admin with roles.manage =="
code=$(req POST /tenants/acme/roles "$MALLORY" '{"key":"backdoor","name":"Backdoor","permissions":["tenant.delete"]}')
check "admin minting a role with tenant.delete -> 403" 403 "$code"
echo "        error: $(jqr .error)"

code=$(req POST /tenants/acme/roles "$MALLORY" '{"key":"sneaky","name":"Sneaky","permissions":["members.read","audit.read","tenant.delete"]}')
check "...even hidden among permissions she holds -> 403" 403 "$code"

MALLORY_ID=$(uid "$MALLORY")
code=$(req PUT "/tenants/acme/members/$MALLORY_ID/roles" "$MALLORY" "{\"role_ids\":[\"$OWNER_ID\"]}")
check "admin assigning herself the owner role -> 403" 403 "$code"

code=$(req POST /tenants/acme/invitations "$MALLORY" "{\"email\":\"accomplice@example.com\",\"role_id\":\"$OWNER_ID\"}")
check "admin inviting an accomplice as owner -> 403" 403 "$code"

ALICE_ID=$(uid "$ALICE")
code=$(req PUT "/tenants/acme/members/$ALICE_ID/roles" "$MALLORY" "{\"role_ids\":[\"$MEMBER_ID\"]}")
check "admin demoting the owner -> 403" 403 "$code"
code=$(req DELETE "/tenants/acme/members/$ALICE_ID" "$MALLORY")
check "admin removing the owner -> 403" 403 "$code"

echo "== but she CAN build roles within her own authority =="
code=$(req POST /tenants/acme/roles "$MALLORY" '{"key":"auditor","name":"Auditor","permissions":["tenant.read","audit.read"]}')
...
BOB_ID=$(uid "$BOB")
code=$(req PUT "/tenants/acme/members/$BOB_ID/roles" "$MALLORY" "{\"role_ids\":[\"$MEMBER_ID\",\"$AUDITOR_ID\"]}")
...
code=$(req PUT "/tenants/acme/roles/$ADMIN_ID" "$ALICE" '{"name":"Hijacked","permissions":["tenant.read"]}')
check "owner editing the system 'admin' role -> 403" 403 "$code"
code=$(req DELETE "/tenants/acme/roles/$ADMIN_ID" "$ALICE")
check "owner deleting a system role -> 403" 403 "$code"
code=$(req POST /tenants/acme/roles "$ALICE" '{"key":"admin","name":"My Admin","permissions":["tenant.read"]}')
check "reusing a system role's key -> 409" 409 "$code"

echo "== a role in use cannot be deleted =="
code=$(req DELETE "/tenants/acme/roles/$AUDITOR_ID" "$ALICE")
check "deleting a role bob still holds -> 409" 409 "$code"
code=$(req PUT "/tenants/acme/members/$BOB_ID/roles" "$ALICE" "{\"role_ids\":[\"$MEMBER_ID\"]}")
...
code=$(req POST /tenants/acme/roles "$ALICE" '{"key":"fake","name":"Fake","permissions":["billing.refund"]}')
check "role with an invented permission -> 400" 400 "$code"
echo "        error: $(jqr .error)"

echo "== invariants =="
code=$(req PUT "/tenants/acme/members/$ALICE_ID/roles" "$ALICE" "{\"role_ids\":[\"$ADMIN_ID\"]}")
check "sole owner stripping her own owner role -> 409" 409 "$code"
code=$(req PUT "/tenants/acme/members/$BOB_ID/roles" "$ALICE" '{"role_ids":[]}')
check "leaving a member with zero roles -> 400" 400 "$code"

echo "== cross-tenant role isolation =="
req POST /tenants "$BOB" '{"slug":"globex","name":"Globex"}' >/dev/null
code=$(req POST /tenants/globex/roles "$BOB" '{"key":"secret","name":"Secret","permissions":["audit.read"]}')
check "bob creates a role in HIS tenant" 201 "$code"
SECRET_ID=$(jqr .id)
# Alice is an OWNER of acme (all permissions) — only tenant scoping can stop her.
code=$(req PUT "/tenants/acme/members/$BOB_ID/roles" "$ALICE" "{\"role_ids\":[\"$MEMBER_ID\",\"$SECRET_ID\"]}")
check "acme's owner assigning globex's role -> 404" 404 "$code"
code=$(req PUT "/tenants/acme/roles/$SECRET_ID" "$ALICE" '{"name":"Hijacked","permissions":["tenant.read"]}')
check "acme's owner editing globex's role -> 404" 404 "$code"
code=$(req DELETE "/tenants/acme/roles/$SECRET_ID" "$ALICE")
