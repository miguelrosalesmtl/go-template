...
// person can act outside a tenant they belong to. These tests pin down exactly
...
func TestSuperuserBypassesTenantMembership(t *testing.T) {
	svc := newTestService(t)
	ctx := context.Background()

	acme, _, alice, _ := setupTwoTenants(t, svc)
	root := makeSuperuser(t, svc, "root@example.com")

	t.Run("enters a tenant it has no membership in", func(t *testing.T) {
		access, err := svc.ResolveTenant(ctx, root, "acme")
		if err != nil {
	...
	t.Run("an ordinary user still cannot", func(t *testing.T) {
		...
		if _, err := svc.ResolveTenant(ctx, alice, "globex"); !errors.Is(err, ErrNotFound) {
			t.Errorf("a non-superuser reached another tenant: got %v, want ErrNotFound", err)
		}
	})

	t.Run("the bypass does not conjure tenants that do not exist", func(t *testing.T) {
		if _, err := svc.ResolveTenant(ctx, root, "no-such-tenant"); !errors.Is(err, ErrNotFound) {
			t.Errorf("superuser resolving a nonexistent tenant: got %v, want ErrNotFound", err)
		}
	})

	// A superuser who is a real member of a tenant is just a member there. Their
	// actual role applies, and nothing is flagged -- otherwise every ordinary
	// action they took in their own tenant would be audited as a bypass, burying
	// the accesses that actually matter.
	t.Run("a superuser who IS a member uses their real role", func(t *testing.T) {
		if _, err := svc.CreateTenant(ctx, root, "rootcorp", "Root Corp"); err != nil {
			t.Fatalf("superuser creating their own tenant: %v", err)
		}

		access, err := svc.ResolveTenant(ctx, root, "rootcorp")
		if err != nil {
			t.Fatalf("superuser resolving their own tenant: %v", err)
		}
		if access.ViaSuperuser {
			t.Error("a superuser acting in their OWN tenant was flagged as a bypass")
		}
	...
	t.Run("the bypass is audited", func(t *testing.T) {
		if err := svc.RecordSuperuserAccess(ctx, root, acme, "GET", "/api/v1/tenants/acme/members"); err != nil {
			t.Fatalf("record superuser access: %v", err)
		}

		var found bool
		for _, e := range listAudit(t, acme.ID) {
			if e.Action == "superuser.tenant_accessed" {
				found = true
			}
		}
		if !found {
			t.Error("no superuser.tenant_accessed entry in the tenant's audit log")
		}
	})
}
...
func TestSuperuserGrantAndRevoke(t *testing.T) {
	...
	t.Run("the grant is audited with a null actor", func(t *testing.T) {
		...
		// tenant_id is NULL -- this is an installation-wide act, not a tenant one.
		var n int
		err := testPool.QueryRow(ctx,
			`SELECT count(*) FROM audit_log
			 WHERE action = 'superuser.granted'
			   AND actor_user_id IS NULL
			   AND tenant_id IS NULL`).Scan(&n)
		if err != nil {
